...
def get_transform(train):
    ...
    if train:
        ...
        #transforms.append(T.ScaleJitter((height, weight)))
        #transforms.append(T.RandomIoUCrop())
    return T.Compose(transforms)

## train loop ###
def main():
    print("training started: rcnn with no data augs.") #update
    # train on the GPU or on the CPU, if a GPU is not available
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    print(f'using {device} to train')
    
    # Define a directory to save checkpoints
    # checkpoint_dir = '/home/tamiroffen/mini_project/project/checkpoints/rcnn_no_aug'
    ...
    dataset = TacoDataset(dataset_path, get_transform(train=False), imgs_path) 
    ...
    # tensorboard
    writer = SummaryWriter("/home/tamiroffen/mini_project/project/runs/rcnn_no_aug")
    step = 0
