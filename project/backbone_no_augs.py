...
## train loop ###
def main():
    print("training started: backbone with no data augs.") #update
    # train on the GPU or on the CPU, if a GPU is not available
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    print(f'using {device} to train')
    
    # Define a directory to save checkpoints
    checkpoint_dir = cwd + '/checkpoints/backbone_no_aug'
    ...
    # tensorboard
    writer = SummaryWriter("/home/tamiroffen/mini_project/project/runs/backbone_no_aug")
    step = 0
